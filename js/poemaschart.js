...
function processTelescopeData(data) {
    ...
    return lastHourData.map(point => ({
        ...
        channel1: point.sfuChannel1, // 45 GHz
        channel2: point.sfuChannel2  // 90 GHz
    }));
}
...
async function createChart() {
    ...
    d3.select("#poemas").select("svg").remove();

    const { width, height, margin } = getChartDimensions('poemas');

    const svg = d3.select("#poemas")
        .append("svg")
    ...
    svg.append("path")
        .datum(data)
        .attr("class", "line-45ghz")
        .attr("d", line1);

    svg.append("path")
        .datum(data)
        .attr("class", "line-90ghz")
        .attr("d", line2);
...
document.addEventListener('DOMContentLoaded', function () {
    function updateDateTime() {
        ...
        document.getElementById('updateTime').textContent = `Updated on: ${formattedDateTime}`;
    }
